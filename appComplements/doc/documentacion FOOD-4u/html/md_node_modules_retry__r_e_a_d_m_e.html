...
<script type="text/javascript">
/* @license magnet:?xt=urn:btih:cf05388f2679ee054f2beb29a391d25f4e673ac3&amp;dn=gpl-2.0.txt GPL-v2 */
$(document).ready(function(){initNavTree('md_node_modules_retry__r_e_a_d_m_e.html',''); initResizable(); });
/* @license-end */
</script>
<div id="doc-content">
...
<div class="contents">
<div class="textblock"><p><a href="http://travis-ci.org/tim-kos/node-retry" title="Check this project&apos;s build status on TravisCI"><img src="https://secure.travis-ci.org/tim-kos/node-retry.png?branch=master" alt="Build Status" class="inline"/></a> <a href="https://codecov.io/gh/tim-kos/node-retry"><object type="image/svg+xml" data="https://codecov.io/gh/tim-kos/node-retry/branch/master/graph/badge.svg" style="pointer-events: none;">codecov</object></a></p>
<h1><a class="anchor" id="autotoc_md17042"></a>
retry</h1>
<p>Abstraction for exponential and custom retry strategies for failed operations.</p>
<h2><a class="anchor" id="autotoc_md17043"></a>
Installation</h2>
<pre class="fragment">npm install retry
</pre><h2><a class="anchor" id="autotoc_md17044"></a>
Current Status</h2>
<p>This module has been tested and is ready to be used.</p>
<h2><a class="anchor" id="autotoc_md17045"></a>
Tutorial</h2>
...
<div class="fragment"><div class="line">var dns = require(&#39;dns&#39;);</div>
...
<div class="fragment"><div class="line">var operation = retry.operation({</div>
...
<div class="line">});</div>
</div><!-- fragment --><h2><a class="anchor" id="autotoc_md17046"></a>
API</h2>
<h3><a class="anchor" id="autotoc_md17047"></a>
retry.operation([options])</h3>
<p>Creates a new <code>RetryOperation</code> object. <code>options</code> is the same as <code>retry.timeouts()</code>'s <code>options</code>, with two additions:</p>
<ul>
<li><code>forever</code>: Whether to retry forever, defaults to <code>false</code>.</li>
<li><code>unref</code>: Whether to <a href="https://nodejs.org/api/timers.html#timers_unref">unref</a> the setTimeout's, defaults to <code>false</code>.</li>
<li><code>maxRetryTime</code>: The maximum time (in milliseconds) that the retried operation is allowed to run. Default is <code>Infinity</code>. <br  />
</li>
</ul>
<h3><a class="anchor" id="autotoc_md17048"></a>
retry.timeouts([options])</h3>
...
<ul>
<li><code>retries</code>: The maximum amount of times to retry the operation. Default is <code>10</code>. Seting this to <code>1</code> means <code>do it once, then retry it once</code>.</li>
...
<div class="fragment"><div class="line">Math.min(random * minTimeout * Math.pow(factor, attempt), maxTimeout)</div>
...
<h3><a class="anchor" id="autotoc_md17049"></a>
retry.createTimeout(attempt, opts)</h3>
<p>Returns a new <code>timeout</code> (integer in milliseconds) based on the given parameters.</p>
<p><code>attempt</code> is an integer representing for which retry the timeout should be calculated. If your retry operation was executed 4 times you had one attempt and 3 retries. If you then want to calculate a new timeout, you should set <code>attempt</code> to 4 (attempts are zero-indexed).</p>
<p><code>opts</code> can include <code>factor</code>, <code>minTimeout</code>, <code>randomize</code> (boolean) and <code>maxTimeout</code>. They are documented above.</p>
<p><code>retry.createTimeout()</code> is used internally by <code>retry.timeouts()</code> and is public for you to be able to create your own timeouts for reinserting an item, see <a href="https://github.com/tim-kos/node-retry/issues/13">issue #13</a>.</p>
<h3><a class="anchor" id="autotoc_md17050"></a>
retry.wrap(obj, [options], [methodNames])</h3>
<p>Wrap all functions of the <code>obj</code> with retry. Optionally you can pass operation options and an array of method names which need to be wrapped.</p>
<div class="fragment"><div class="line">retry.wrap(obj)</div>
<div class="line"> </div>
<div class="line">retry.wrap(obj, [&#39;method1&#39;, &#39;method2&#39;])</div>
<div class="line"> </div>
<div class="line">retry.wrap(obj, {retries: 3})</div>
<div class="line"> </div>
<div class="line">retry.wrap(obj, {retries: 3}, [&#39;method1&#39;, &#39;method2&#39;])</div>
</div><!-- fragment --><p>The <code>options</code> object can take any options that the usual call to <code>retry.operation</code> can take.</p>
<h3><a class="anchor" id="autotoc_md17051"></a>
new RetryOperation(timeouts, [options])</h3>
...
<li><code>forever</code>: Whether to retry forever, defaults to <code>false</code>.</li>
<li><code>unref</code>: Wether to <a href="https://nodejs.org/api/timers.html#timers_unref">unref</a> the setTimeout's, defaults to <code>false</code>.</li>
</ul>
<p>If <code>forever</code> is true, the following changes happen:</p><ul>
<li><code>RetryOperation.errors()</code> will only output an array of one item: the last error.</li>
<li><code>RetryOperation</code> will repeatedly use the <code>timeouts</code> array. Once all of its timeouts have been used up, it restarts with the first timeout, then uses the second and so on.</li>
</ul>
<h4><a class="anchor" id="autotoc_md17052"></a>
retryOperation.errors()</h4>
<p>Returns an array of all errors that have been passed to <code>retryOperation.retry()</code> so far. The returning array has the errors ordered chronologically based on when they were passed to <code>retryOperation.retry()</code>, which means the first passed error is at index zero and the last is at the last index.</p>
<h4><a class="anchor" id="autotoc_md17053"></a>
retryOperation.mainError()</h4>
...
<h4><a class="anchor" id="autotoc_md17054"></a>
retryOperation.attempt(fn, timeoutOps)</h4>
...
<h4><a class="anchor" id="autotoc_md17055"></a>
retryOperation.try(fn)</h4>
<p>This is an alias for <code>retryOperation.attempt(fn)</code>. This is deprecated. Please use <code>retryOperation.attempt(fn)</code> instead.</p>
<h4><a class="anchor" id="autotoc_md17056"></a>
retryOperation.start(fn)</h4>
<p>This is an alias for <code>retryOperation.attempt(fn)</code>. This is deprecated. Please use <code>retryOperation.attempt(fn)</code> instead.</p>
<h4><a class="anchor" id="autotoc_md17057"></a>
retryOperation.retry(error)</h4>
...
<h4><a class="anchor" id="autotoc_md17058"></a>
retryOperation.stop()</h4>
<p>Allows you to stop the operation being retried. Useful for aborting the operation on a fatal error etc.</p>
<h4><a class="anchor" id="autotoc_md17059"></a>
retryOperation.reset()</h4>
<p>Resets the internal state of the operation object, so that you can call <code>attempt()</code> again as if this was a new operation object.</p>
<h4><a class="anchor" id="autotoc_md17060"></a>
retryOperation.attempts()</h4>
<p>Returns an int representing the number of attempts it took to call <code>fn</code> before it was successful.</p>
<h2><a class="anchor" id="autotoc_md17061"></a>
License</h2>
<p>retry is licensed under the MIT license.</p>
<h1><a class="anchor" id="autotoc_md17062"></a>
Changelog</h1>
<p>0.10.0 Adding <code>stop</code> functionality, thanks to @maxnachlinger.</p>
<p>0.9.0 Adding <code>unref</code> functionality, thanks to @satazor.</p>
